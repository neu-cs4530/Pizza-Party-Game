...
export default function Order({order} : OrderProps): JSX.Element {
  ...
  return( 
    <div style={{backgroundImage: speechBubble,   backgroundPosition: 'center',
    backgroundSize: 'cover', backgroundColor: "white",     borderRadius: 50}}>
      <h1>Order</h1>
      <div>
        {order.pizzas.map(pizza => pizzaDisplay(pizza))}
      </div>
    </div>
  )
}